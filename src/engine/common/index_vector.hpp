...
struct Vector : public GenericProvider
{
    ...
    template<typename U>
        std::enable_if_t<std::is_assignable<T&, U&&>::value, ID>
        push_back(U&& obj);
    [[nodiscard]]
    ID                 getNextID() const;
    void               erase(ID id);
...
template<typename T>
template<typename U>
inline std::enable_if_t<std::is_assignable<T&, U&&>::value, uint64_t>
Vector<T>::push_back(U&& obj){
    const Slot slot = getSlot();
    data[slot.data_id] = std::forward<U>(obj);
    return slot.id;
}
